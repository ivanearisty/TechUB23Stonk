...
def percent_change_margin_by_date_timeframe(ticker, n, stock_return, margin, start_date, end_date): 
  ...
  df["returns"] = df["5. adjusted close"].pct_change(periods=n)*100
  df_sub = df.loc[start_date:end_date]
  df_sub = df_sub[(df["returns"] < stock_return+margin) & (df["returns"] > stock_return-margin)] #defining returns you want 
  finaldict = df_sub[["returns"]].sort_index(ascending=False).to_dict() #sort values by date  #sort_values(["returns"])
  f = finaldict['returns']
...
def bottom_N_returns(ticker, n, t, start_date, end_date): #lowest returns over a certain period within a certain timeframe 
  assert pd.to_datetime(start_date) < pd.to_datetime(end_date)
  df = TimeSeries(key='HY5IIUWSUZSBEEU5', output_format='pandas').get_daily_adjusted(ticker, outputsize='full')[0]
  df=df.sort_index()
  df["returns"] = df["5. adjusted close"].pct_change(periods=n)*100
...
def return_info(ticker, start_date, end_date): #annualized returns, volatility, and sharpe over a certain timeframe 
  """
  all in percentages of 100
  """
  ...
  df = TimeSeries(key='HY5IIUWSUZSBEEU5', output_format='pandas').get_daily_adjusted(ticker, outputsize='full')[0] #1st input of list 
  ...
  mean_returns = df_sub.mean()*252 # annulizing number of trading days in a year. average daily return*252
  volatility = df_sub.std()*np.sqrt(252) #volatility is standard deviation of annualized returns*square root of 252, np is numpy library  
  finaldict = pd.DataFrame({"Annualized Returns":[mean_returns], "Annualized Volatility":[volatility], "Annualized Sharpe": [mean_returns/volatility]}).to_dict()
